 ...
 *
 * Status of what's actually verified vs. still assumed, as of this
 * conversation:
 *
 * 1. NAMESPACE ("http://tempuri.org/") — verified: every SOAP action string
 *    embedded in the decompiled exe uses this namespace.
 * 2. IIS virtual directory name ("InventoryManage") — verified: the admin
 *    web page's WebResource.axd loads from "/InventoryManage/", confirmed
 *    directly from that page's HTML source.
 * 3. The exact .asmx filename ("Service.asmx") — still an assumption, not
 *    verified anywhere. Please confirm by browsing
 *    http://192.168.0.22/InventoryManage/ directly.
 * 4. Exact SOAP parameter names/shapes (e.g. whether CheckUserLogin2 takes
 *    flat scalar params or a single complex User object) — NOT verified.
 *    No live call has actually been made against the server in this
 *    project yet. Treat the parameter names in AuthRepository /
 *    ProductRepository as a reasonable first guess to test, not as fact.
 * 5. Device binding — verified real: the admin site has a grid mapping
 *    each login to a device GUID with a "ResetDevices" action. This means
 *    deviceID sent from Android MUST be stable across app launches (e.g.
 *    persisted once via Settings.Secure.ANDROID_ID, not regenerated).
 */
object SoapClient {
     ...
     * tested against the live server — if it fails with a SOAP fault
     * about an unrecognized parameter, that's expected on the first try.
     */
